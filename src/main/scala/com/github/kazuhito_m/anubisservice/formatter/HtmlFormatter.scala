...
class HtmlFormatter extends BaseFormatter {
  ...
  def analyzeEndCellCount(rootCell: Cell): Int = {
    ...
    if (map.isEmpty) 1
    else map.values.foldLeft(0) {
      (count, item) => count + analyzeEndCellCount(item)
    }
  }

  def getLastCell(cell: Cell): Cell = cell.children.values.foldLeft(cell) {
    (last, cur) => getLastCell(cur)
  }
  ...
  def createTableDetail(cell: Cell, lastCell: Cell): String = {
    ...
    // 組立とreturn
    tdTag + trTags + map.values.foldLeft("") {
      (html, item) => html + createTableDetail(item, lastCell)
    }
  }
